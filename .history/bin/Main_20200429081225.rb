...
class Main
  ...
  def show_rules(emoji.codes)
    enter = "\n\n"
    show_example_position emoji.codes
  ...
  def select_rules(human, emoji.codes)
    print "Awesome! #{human}\n Do you know the rules? [Y / N]: "
    choice = gets.chomp.downcase.to_sym
    show_rules(emoji.codes) if choice == :n
    print "\n Good luck, Human!\n"
    # sleep(5)
  end

  def get_x(human, machine, emoji.codes)
    (human == :x || machine == :x) && emoji_friendly? ? emoji.codes[:x] : 'x'
  end

  def get_o(human, machine, emoji.codes)
    (human == :o || machine == :o) && emoji_friendly? ? emoji.codes[:o] : 'o'
  end

  # return the hexadecimal representation of my next move
  def next_move(human, machine, move, emoji.codes)
    if move.odd?
  ...
  def play(board, version, emoji)
    ...
    select_rules(human, emoji.codes)
    moves = board.moves
    ...
    until game_over? moves
      ...
      if valid?(position, board)
        moves += 1
        move = next_move(human, machine, moves, emoji.codes)

        board.cell[position - 1] = move
      end
      show(board, emoji.codes)
      clear
    end
  end
end
...
game = Main.new
board = Board.new
codex = EmojiUnicode.new
emoji = Emoji.new(codex)
