...
class Main
  ...
  def show(board, emojj, variant = nil)
    str = "\n"
    board = board.cell
  ...
  def show_example_position(codes, emoji)
    # binding.pry
    number_list = ('1'.ord..'9'.ord).to_a
    emoji_list = []

    number_list.each do |base|
      emoji_list << to_emoji(base, codes[:emoji_style], codes[:box]) if emoji.friendly?
    end

    show(emoji_list, codes, codes[:emoji_style])
  end

  def show_rules(codes, emoji)
    enter = "\n\n"
    show_example_position(codes,  emoji)
  ...
  def select_rules(human, emoji)
    ...
    show_rules(emoji.codes, emoji) if choice == :n
    print "\n Good luck, Human!\n"
    # sleep(5)
  end
